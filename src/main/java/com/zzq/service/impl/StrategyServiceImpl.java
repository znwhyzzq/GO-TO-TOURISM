...
import com.zzq.mapper.StrategyMapper;
...
public class StrategyServiceImpl extends ServiceImpl<StrategyMapper, Strategy> implements StrategyService {
    ...
    public ResponseResult collectStrategy(Long id) {
        ...
        Date date = new Date();
        //2.判断当前登录用户是否已经收藏
        String key = CACHE_COLLECT_KEY + userId + ":" + id;
        Boolean member = redisTemplate.opsForSet().isMember(key, userId.toString());
        if (BooleanUtil.isFalse(member)) {
            //3.如果未收藏，可以收藏
            //3.1数据库收藏数+1
            ...
            boolean remove = collectionService.remove(new LambdaQueryWrapper<Collection>()
                    .eq(Collection::getProductId, id)
                    .eq(Collection::getProductType, 5)
            );
            if (!remove) {
    ...
    private void isCollect(Strategy strategy) {
        //1.获取登录用户
        Long userId = WebUtils.getUserId();
        String key = CACHE_COLLECT_KEY +  userId + ":" + strategy.getId();
        Boolean isMember = redisTemplate.opsForSet().isMember(key, userId.toString());
