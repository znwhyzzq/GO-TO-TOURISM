...
import static com.zzq.constant.RedisConstants.*;
...
public class OrderServiceImpl extends ServiceImpl<OrderMapper, Order> implements OrderService {
    ...
    public Integer count0() {
        String key = CACHE_ORDER_COUNT0_KEY;
    ...
    public Boolean toBePaid(Order order) {
        ...
        if (order.getProductType() == 4) {
            ...
            boolean update = update(order, lambdaQueryWrapper1);
            if (update) {
    ...
    @Override
    public OrderListV0 orderListPage(UserOrderQuery query) {
        Long userId = WebUtils.getUserId();
        ...
        if (CollectionUtils.isEmpty(result.getRecords())) {
            return new OrderListV0(0.0, Collections.emptyList(), 0L);
        }
        //遍历，支付状态为已支付累加
        double totalFee = result.getRecords()
                .stream()
                .filter(order -> order.getState() == 1)
                .mapToDouble(Order::getFee)
                .sum();

        return new OrderListV0(totalFee, result.getRecords(), result.getTotal());
    }
    ...
    public Integer count1() {
        String key = CACHE_ORDER_COUNT1_KEY;
        if (redisTemplate.hasKey(key)) {
            String count = redisTemplate.opsForValue().get(key);
            Integer count0 = (Integer) JSON.parse(count);
            log.info("走缓存.....");
    ...
    public Boolean deleteById(Long id) {
        String count0Key = CACHE_ORDER_COUNT0_KEY;
        String count1Key = CACHE_ORDER_COUNT1_KEY;
        String orderKey = CACHE_ORDER_KEY + id;
    ...
    //查询最受人欢迎的景区,酒店，出行，路线
    @Override
    public List<AnalysisVO> attractions() {
        QueryWrapper<Order> queryWrapper = new QueryWrapper<>();
        queryWrapper.select("product_name as product_name_alias", "sum(fee) as total_fee")
                .eq("DELETE_STATUS", 0)
        ...
        List<AnalysisVO> analysisVOList = results.stream().map(result -> {
            AnalysisVO vo = new AnalysisVO();
            vo.setKey((String) result.get("product_name_alias"));
            vo.setValue(((Double) result.get("total_fee")).intValue());
            return vo;
    ...
    @Override
    public List<AnalysisVO> getOrderPayedTotal() {
        QueryWrapper<Order> queryWrapper = new QueryWrapper<>();
        queryWrapper.select("PRODUCT_TYPE", "sum(FEE) as total_fee")
                .eq("DELETE_STATUS", 0)
                //.eq("STATE", 2)
        ...
        return results.stream()
                .map(result -> {
                    ...
                    analysisVO.setKey(result.get("PRODUCT_TYPE").toString());
                    analysisVO.setValue((int) Math.round((Double) result.get("total_fee")));
                    return analysisVO;
                })
                .collect(Collectors.toList());
    }
    ...
    public List<Order> findAll() {
        String key = CACHE_ORDERALLTOBYPAY_KEY;
    ...
    @Override
    public OrderListV0 listPage(OrderQuery query) {
        //分页
        Page<Order> page = new Page<>();
        page.setCurrent(query.getCurrentPage());
        page.setSize(query.getPageSize());
