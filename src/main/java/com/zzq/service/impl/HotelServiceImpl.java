...
import com.zzq.mapper.HotelMapper;
import com.zzq.qo.HotelQuery;
import com.zzq.service.HotelService;
...
public class HotelServiceImpl extends ServiceImpl<HotelMapper, Hotel> implements HotelService {
    ...
    public Integer count0() {
        String key = CACHE_HOTEL_COUNT0_KEY;
    ...
    public Integer count1() {
        String key = CACHE_HOTEL_COUNT1_KEY;
        String count = redisTemplate.opsForValue().get(key);
        if (StringUtils.isNotBlank(count)) {
            Integer count1 = (Integer) JSON.parse(count);
            log.info("走缓存");
    ...
    public void saveOrUpdateHotel(Hotel hotel) throws Exception {
        ...
        if (id == null) {
            ...
            //2.删除缓存
            redisTemplate.delete(CACHE_HOTEL_KEY + id);
        }
    }

    @Override
    public Hotel findHotelById(Long id) throws Exception {
        String key = CACHE_HOTEL_KEY + id;
    ...
    public void deleteById(Long id) {
        ...
        redisTemplate.delete(CACHE_HOTEL_KEY + id);
        redisTemplate.delete(CACHE_HOTEL_COUNT1_KEY);
        redisTemplate.delete(CACHE_HOTEL_COUNT0_KEY);
    }
    ...
    public Page<Hotel> listPage(HotelQuery query) {
        ...
        lambdaQueryWrapper.like(StringUtils.isNotBlank(query.getAddress()), Hotel::getAddress, query.getAddress());
