...
import com.zzq.qo.AdminQuery;
import com.zzq.qo.UserQuery;
...
public class AdminServiceImpl extends ServiceImpl<AdminMapper, Admin> implements AdminService {
    ...
    public Integer count1() {
        String key = CACHE_ADMIN_COUNT0_KEY;
    ...
    public List<Map<String, Object>> findProvinceByid() throws Exception {
        ...
        for (ProvinceVo provinceVo : porvicesList) {
            ...
            map0.put("省份", provinceVo.getAddr());
            map0.put("注册用户", provinceVo.getCount());
            mapList.add(map0);
        }
        return mapList;
    }
    ...
    public void saveOrUpdateUser(Admin admin) throws Exception {
        String userKey = CACHE_ADMIN_KEY + admin.getId();
        Date date = new Date();
        if (admin.getId() == null) {
    ...
    public void sendEmail(Admin admin) {
        ...
        // 邮件主题
        mailMessage.setSubject("欢迎注册旅游网，这是您的账号信息");
        // 邮件的文本信息
    ...
    public ResponseResult saveOrUpdateAdmin(Admin admin) {
        //String key = CACHE_ADMIN_KEY + admin.getId();
        Date date = new Date();
        if (admin.getId() == null) {
            //新增管理员
            admin.setUserType(1);
            ...
            System.out.println(admin.getIcon());
    ...
    public void deleteById(Long id) throws Exception {
        String adminKey = CACHE_ADMIN_KEY + id;
        adminMapper.deleteById(id);
        redisTemplate.delete(CACHE_ADMIN_COUNT0_KEY);
        redisTemplate.delete(CACHE_ADMIN_COUNT1_KEY);
        redisTemplate.delete(adminKey);
    }

    @Override
    public void updateStates(Admin admin) throws Exception {
        String adminKey = CACHE_ADMIN_KEY + admin.getId();
    ...
    public ResponseResult updatePassword(AdminDTO adminDTO) {
        Long userId = WebUtils.getUserId();
        Admin user = getById(userId);
        String adminKey = CACHE_ADMIN_KEY + user.getId();
        ...
        //2.判断输入密码是否与数据库匹配
        if (passwordEncoder.matches(adminDTO.getOldPassword(), user.getPassword())) {
            //3.获取用户输入新密码加密
